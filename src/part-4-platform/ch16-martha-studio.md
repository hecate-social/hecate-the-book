...
Martha event-sources everything. Every LLM call, every gate decision, every compile result, every human rejection — all of it lands in the event store as immutable history. Right now those events serve two purposes: they drive the real-time UI (as described above), and they provide an audit trail you can replay. Useful. But also wasteful, because that event stream contains something far more valuable than history. It contains fitness signals.

Every gate you reject teaches Martha what you don't accept. Every model selection that produces clean code on the first try teaches it which models suit which tasks. Every pipeline run where half the agents produce nothing useful teaches it which agents to skip. The data is already there, accumulating with every session. The obvious next step is closing the loop: feeding those events into evolved neural networks that improve Martha's decisions over time.

Five components, each evolved independently:

**Triage Network.** A 50-neuron TWEANN (Topology and Weight Evolving Artificial Neural Network, from faber-tweann) that learns which LLM model to use for which task. The hand-tuned scoring function described earlier — flagship for architecture, fast for boilerplate — works well enough. But "well enough" means it still sends Opus to write getter functions and Haiku to design aggregate boundaries more often than you'd like. The triage network replaces those static rules with a topology that evolves against a single fitness metric: quality-to-cost ratio derived from past decisions. Quality is measured by gate pass rates and downstream compile success. Cost is measured in tokens and latency. The network that picks the cheapest model whose output you actually accept wins.

**Gate Predictor.** An evolved classifier that learns when to present a gate and when to skip it. This one carries an asymmetric fitness penalty — skipping a gate that you would have rejected costs ten times more than showing a gate you would have passed. The penalty is deliberately harsh. Martha should err on the side of asking. Over weeks of use, the predictor learns your rejection patterns: you always reject single-aggregate designs, you never reject event naming conventions, you sometimes reject bounded context boundaries. The gates you never reject quietly disappear. The gates you care about stay. Your Martha develops a feel for your taste.

**Prompt Evolver.** A population of prompt strategies competing via generational evolution. Each individual in the population is a prompt template with structural parameters — how much context to include, which examples to reference, how to frame constraints, where to place the critical instruction. Mutation operators map to prompt structure changes: swap example order, add a constraint paragraph, remove redundant context, rephrase the system prompt. Crossover combines structural elements from two parents. The population evolves against agent output quality as judged by gate pass rates and compile success. Winners propagate. Losers are culled. After a hundred generations, the surviving prompts bear little resemblance to the hand-written originals — and they work better, for your specific codebase and your specific standards.

**Pipeline Skipper.** Martha's venture lifecycle runs twelve agent roles in sequence. For a simple rename, eleven of those roles produce nothing useful — you only need the coder. For a multi-file refactor touching three bounded contexts, you need the full pipeline. The pipeline skipper is an evolved network that learns which roles to activate for which task type. Its fitness function is straightforward: minimize total pipeline time while maintaining gate pass rate. A network that skips agents whose output you consistently wave through without reading scores higher than one that runs everything every time.

**Meta-Controller.** The four networks above each optimize a narrow concern. The meta-controller — built on faber-neuroevolution's Liquid Conglomerate architecture (L2 networks with Long-Term Credit neurons) — balances them across weeks and months. When the triage network starts over-optimizing for cost and quality drops, the meta-controller adjusts. When the gate predictor gets too aggressive about skipping and a bad decision slips through, the meta-controller tightens the asymmetric penalty. It operates on a longer timescale than the individual networks, evolving against a composite fitness that weights quality, cost, speed, and user satisfaction.

The key insight is simple, and it's the reason Martha takes another path from every other AI coding tool on the market. Every other tool is stateless. Every session starts from zero. They never learn your patterns, your preferences, your codebase's idioms. They treat every developer and every project identically, because they have no memory and no feedback loop.

Martha evolves against your actual decisions. The event store is the memory. The evolved networks are the feedback loop. The longer you use it, the better it gets — for your codebase, your taste, your standards. Nobody else's Martha behaves like yours, because nobody else has your event history.

While everyone else in the industry is building bigger foundation models and hoping that scale solves everything, we're evolving smaller networks that know when to call the big models and when not to bother. A 50-neuron TWEANN that has evolved against six months of your gate decisions will outperform any hand-tuned heuristic for model selection — not because it's smarter in general, but because it's smarter about you in particular.

And everything stays local. The event store lives on your machine. The evolution runs on your hardware. The neural networks are yours. No telemetry, no cloud training, no "improving our models with your data." Privacy by architecture, not by policy. The same principle that makes the event store trustworthy makes the evolution trustworthy — it never leaves your machine, so you never have to wonder who else is learning from your decisions.
